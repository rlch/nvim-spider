...
local punctuationPattern = "%p+"

---@class optsObj
---@field skipInsignificantPunctuation boolean defaults to true
---@field punctuationPattern string defaults to %p+
...
function M.setup(opts)
  ...
  if opts.punctuationPattern ~= nil then
    punctuationPattern = opts.punctuationPattern
  end
end
...
local function getNextPosition(line, col, key)
  ...
  local patterns = {
    ...
    punctuation = "%f[^%s]" .. punctuationPattern .. "%f[%s]", -- punctuation surrounded by whitespace
    punctAtStart = "^" .. punctuationPattern .. "%f[%s]",      -- needed since lua does not allow for logical OR
    punctAtEnd = "%f[^%s]" .. punctuationPattern .. "$",
    onlyPunct = "^" + punctuationPattern + "$",
  }
  if not skipInsignificantPunc then patterns.punctuation = punctuationPattern end
